...
st.set_page_config(page_title="🧠 Meme Coin Radar v6.1.5", layout="wide")
st.title("🧠 Meme Coin Radar v6.1.5")
st.markdown("🔥 Multi-API fallback • Expanded coin list • Smarter display & caching")
...
def fetch_coingecko_price(id):
    try:
        ...
        if id not in d:
            return None
        price = d[id]["usd"]
        change = d[id].get("usd_24h_change", 0)
        return {
            "name": id.capitalize(),
            "symbol": id.upper(),
            "priceUsd": price,
