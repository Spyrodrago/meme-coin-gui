...
st.set_page_config(page_title="🧠 Meme Coin Radar v5.3", layout="wide")
st.title("🧠 Meme Coin Radar v5.3")
st.markdown("Now with fallback price logic, error messages, and expanded trending cryptos.")
...
def get_dexscreener_data(pair):
    try:
        ...
        return {
            ...
            "status": "OK" if price else "⚠️ No price data"
        }
...
def get_top_hot_cryptos(limit=15):
    url = "https://api.coingecko.com/api/v3/coins/markets"
    try:
        response = requests.get(url, params={"vs_currency": "usd", "order": "volume_desc", "per_page": 500, "page": 1})
        top = []
        ...
        # Show top by volume + rising coins
        top_sorted = sorted(top, key=lambda x: x['change'], reverse=True)
...
if should_refresh:
    ...
    if view_mode == "🐸 Meme Coins":
        ...
        st.subheader("🔥 Top Trending Cryptos (by Volume & % Gain)")
